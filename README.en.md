...
| Stays on the layer you're on | Every candidate gets a **layer score** (positioned elements, higher z-index and things floating above the document flow score higher), and the D-pad **prefers targets that aren't below the current one**. So a popover opened by a top-bar button won't be stolen by a full-width banner on the same page, and you can walk it all the way down; once it runs out, the content underneath still works even though it scores lower (it's *ranked after*, not excluded). Moving around inside a layer **never scrolls the whole page** (it used to go "flip a screen" looking for something outside, up to four screens at a time, and the page would run away) |
...
The APK is at `dist/tv-browser-1.1.2-release.apk`. Three ways to install it, pick any one:
...
powershell -File tools\build.ps1             # build → dist\tv-browser-1.1.2-release.apk
...
$env:NODE_PATH="C:\nvm4w\nodejs\node_modules"; node test\bridge.mjs  # 57 items: the remote-control bridge contract in Chromium
